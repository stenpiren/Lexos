...
from lexos.processors.prepare.cutter import cut, _cut_by_characters, \
    _cut_by_words, _cut_by_lines, _cut_by_number, _cut_by_milestone


class TestCutByCharacters:
    def test_empty_string(self):
        assert _cut_by_characters(text="", seg_size=10, overlap=5,
                                  last_prop=1) == [""]
        assert _cut_by_characters(text=" ", seg_size=100, overlap=0,
                                  last_prop=0.5) == [""]

    def test_string_seg_size(self):
        assert _cut_by_characters(text="ABABABAB", seg_size=10, overlap=0,
                                  last_prop=1) == ["ABABABAB"]
        assert _cut_by_characters(text="ABABABAB", seg_size=2, overlap=0,
                                  last_prop=1) == ["AB", "AB", "AB", "AB"]
        assert _cut_by_characters(text="ABABABAB", seg_size=3, overlap=0,
                                  last_prop=1) == ["ABA", "BABAB"]
        assert _cut_by_characters(text="A", seg_size=100, overlap=0,
                                  last_prop=5) == ["A"]

    def test_string_overlap(self):
        assert _cut_by_characters(text="WORD", seg_size=2, overlap=0,
                                  last_prop=1) == ["WO", "RD"]
        assert _cut_by_characters(text="ABBA", seg_size=2, overlap=1,
                                  last_prop=1) == ["AB", "BB", "BA"]
        assert _cut_by_characters(text="ABCDE", seg_size=3, overlap=2,
                                  last_prop=1) == ["ABC", "BCD", "CDE"]
        assert _cut_by_characters(text="ABCDEF", seg_size=4, overlap=3,
                                  last_prop=1) == ["ABCD", "BCDE", "CDEF"]

    def test_string_last_prop(self):
        assert _cut_by_characters(text="ABABABABABA", seg_size=5, overlap=0,
                                  last_prop=0.2) == ["ABABA", "BABAB", "A"]
        assert _cut_by_characters(text="ABABABABABA", seg_size=5, overlap=0,
                                  last_prop=0.21) == ["ABABA", "BABABA"]
        assert _cut_by_characters(text="ABABABABABA", seg_size=5, overlap=0,
                                  last_prop=2) == ["ABABABABABA"]
        assert _cut_by_characters(text="ABCDEFGHIJKL", seg_size=3, overlap=0,
                                  last_prop=2) == ["ABC", "DEF", "GHIJKL"]
        assert _cut_by_characters(text="ABCDEFGHIJKL", seg_size=3, overlap=0,
                                  last_prop=5) == ["ABCDEFGHIJKL"]

    def test_string_all_funcs(self):
        assert _cut_by_characters(text="ABABABABABA", seg_size=4, overlap=1,
                                  last_prop=0.5) == \
               ["ABAB", "BABA", "ABAB", "BA"]

    def test_pre_conditions(self):
        try:
            _ = _cut_by_characters(text="ABAB", seg_size=0, overlap=0,
                                   last_prop=1)
        ...
        try:
            _ = _cut_by_characters(text="ABAB", seg_size=2, overlap=-1,
                                   last_prop=1)
        ...
        try:
            _ = _cut_by_characters(text="ABAB", seg_size=2, overlap=0,
                                   last_prop=-1)
            raise AssertionError("None negative error did not raise")
        except AssertionError as error:
            assert str(error) == NEG_OVERLAP_LAST_PROP_MESSAGE
        ...
        try:
            _ = _cut_by_characters(text="ABAB", seg_size=2, overlap=2,
                                   last_prop=1)
...
class TestCutByWords:
    def test_cut_by_words(self):
        assert _cut_by_words(text=" ", seg_size=1, overlap=0,
                             last_prop=1) == [""]
        assert _cut_by_words(text="test test", seg_size=1, overlap=0,
                             last_prop=1) == ["test ", "test"]
        assert _cut_by_words(text="abc abc abc abc abc abc abc abc abc abc abc "
    ...
    def test_cut_by_words_no_whitespace(self):
        assert _cut_by_words(text="testtest", seg_size=1, overlap=0,
                             last_prop=1) == ["testtest"]
        assert _cut_by_words(text="helloworld helloworld", seg_size=1,
                             overlap=0, last_prop=1) == [
            "helloworld ", "helloworld"]

    def test_cut_by_words_overlap(self):
        assert _cut_by_words(text="test test test", seg_size=2, overlap=1,
                             last_prop=.5) == ["test test ",
                                              "test test", "test"]

    def test_seg_size_assertion_error(self):
        try:
            _ = _cut_by_words(text="test test", seg_size=1, overlap=1,
                              last_prop=.5)
        ...
        try:
            _ = _cut_by_words(text="test test test", seg_size=1, overlap=2,
                              last_prop=.5)
            raise AssertionError("did not throw error")
        except AssertionError as error:
            assert str(error) == LARGER_SEG_SIZE_MESSAGE
    ...
    def test_cut_by_words_proportion(self):
        assert _cut_by_words(text="test test test", seg_size=2, overlap=0,
        ...
        assert _cut_by_words(text="test test test test", seg_size=2,
                             overlap=0, last_prop=1) == [
            "test test ", "test test"]
        assert _cut_by_words(text="test test test test test", seg_size=2,
                             overlap=0, last_prop=.5) == [
            "test test ", "test test ", "test"]
        assert _cut_by_words(text="test test test test test", seg_size=2,
                             overlap=0, last_prop=1) == [
            "test test ", "test test test"]
        assert _cut_by_words(text="test test test test test", seg_size=3,
                             overlap=0, last_prop=1) == [
            "test test test test test"]

    def test_cut_by_words_zero_chunks_precondition(self):
        try:
            _ = _cut_by_words(text=" ", seg_size=0, overlap=0, last_prop=.5)
            raise AssertionError("zero_division error did not raise")
        except AssertionError as error:
            assert str(error) == NON_POSITIVE_SEGMENT_MESSAGE

        try:
            _ = _cut_by_words(text="test test", seg_size=0, overlap=0,
                              last_prop=.5)
            raise AssertionError("zero_division error did not raise")
        except AssertionError as error:
            assert str(error) == NON_POSITIVE_SEGMENT_MESSAGE
    ...
    def test_cut_by_words_neg_chunk_precondition(self):
        try:
            _ = _cut_by_words(text="test", seg_size=-1, overlap=0,
                              last_prop=.5)
            raise AssertionError("did not throw error")
        except AssertionError as error:
            assert str(error) == NON_POSITIVE_SEGMENT_MESSAGE
    ...
    def test_cut_by_words_neg_overlap_precondition(self):
        try:
            _ = _cut_by_words(text="test", seg_size=1, overlap=-1,
                              last_prop=.5)
            raise AssertionError("did not throw error")
        except AssertionError as error:
            assert str(error) == NEG_OVERLAP_LAST_PROP_MESSAGE
...
class TestCutByLines:
    def test_cut_by_lines_empty(self):
        assert _cut_by_lines(text="", seg_size=1, overlap=0,
                             last_prop=1) == [""]
        assert _cut_by_lines(text="\n", seg_size=1, overlap=0,
                             last_prop=1) == ["\n"]

    def test_cut_by_lines_regular(self):
        assert _cut_by_lines(text="test", seg_size=100, overlap=0,
                             last_prop=0.5) == ["test"]
        assert _cut_by_lines(text="test", seg_size=1,
                             overlap=0, last_prop=1) == ["test"]
        assert _cut_by_lines(text="test\ntest\ntest", seg_size=2,
    ...
    def test_cut_by_lines_line_ending(self):
        assert _cut_by_lines(text="test\rtest", seg_size=1,
                             overlap=0, last_prop=1) == ["test\r", "test"]
        assert _cut_by_lines(text="test\rtest\ntest", seg_size=1,
    ...
    def test_cut_by_lines_zero_seg_size(self):
        try:
            _ = _cut_by_lines(text="", seg_size=0, overlap=0, last_prop=1)
    ...
    def test_cut_by_lines_neg_nums(self):
        try:
            _ = _cut_by_lines(text="", seg_size=1, overlap=-1, last_prop=-1)
    ...
    def test_cut_by_lines_larger_seg_size(self):
        try:
            _ = _cut_by_lines(text="", seg_size=1, overlap=2, last_prop=1)
...
class TestCutByNumbers:
    def test_cut_by_number_normal(self):
        assert _cut_by_number(text="Text", num_segment=1) == ["Text"]

        assert _cut_by_number(text="This text has five words",
                              num_segment=5) == ["This ", "text ", "has ",
                                                "five ", "words"]

        assert _cut_by_number(text="Odd number of words in this text",
                              num_segment=6) == ["Odd number ", "of ",
                                                "words ", "in ", "this ",
                                                "text"]

        assert _cut_by_number(text="Almost enough words here but not quite",
                              num_segment=4) == ["Almost enough ",
                                                "words here ", "but not ",
                                                "quite"]

    def test_cut_by_number_spacing(self):
        assert _cut_by_number(text="Hanging space ", num_segment=2) == [
            "Hanging ", "space "]

        assert _cut_by_number(text="Other  whitespace\n is\tfine!\n\n",
                              num_segment=4) == ["Other  ", "whitespace\n ",
                                                "is\t", "fine!\n\n"]

        assert _cut_by_number(text="      <-There are six spaces here",
                              num_segment=5) == ["<-There ", "are ", "six ",
                                                "spaces ", "here"]
        assert _cut_by_number(text="\n\n\n\n\nword\n\n\n\n\n",
                              num_segment=1) == ["word\n\n\n\n\n"]

    def test_cut_by_number_lines(self):
        assert _cut_by_number(
            text="Latinisalanguagewithnospaces\n"
                 "Youmayfindthisdifficulttoread!",
            num_segment=2) == ["Latinisalanguagewithnospaces\n",
                               "Youmayfindthisdifficulttoread!"]

        assert _cut_by_number(text="line\nline\nline\nline\nline",
                              num_segment=2) == ["line\nline\nline\n",
                                                "line\nline"]

        assert _cut_by_number(text="Languageswithoutanyspacesmayhave\n"
                             "uneven\nchunks", num_segment=3) == \
               ["Languageswithoutanyspacesmayhave\n", "uneven\n", "chunks"]

        assert _cut_by_number(text="Ithinkthisiswhy\u3000Chinesetextcanbesplit",
                              num_segment=2) \
               == ["Ithinkthisiswhy\u3000", "Chinesetextcanbesplit"]

    def test_cut_by_number_excess_chunks(self):
        assert _cut_by_number(text="RemovewhitespaceonChinese?",
                              num_segment=3) == [
            "RemovewhitespaceonChinese?", '', '']
        assert _cut_by_number(text="This text has too few words!",
                              num_segment=8) == \
               ["This ", "text ", "has ", "too ", "few ", "words!", '', '']
        assert _cut_by_number(text="Reeeeeeeeeeeeeeeeeeeeeeeally long word",
                              num_segment=6) == \
    ...
    def test_cut_by_number_bad_math(self):
        ...
        try:
            _ = _cut_by_number(text="Danger zone!", num_segment=0)
        except AssertionError as error:
            assert str(error) == NON_POSITIVE_SEGMENT_MESSAGE
        # Invalid index exception
        try:
            _ = _cut_by_number(text="Oh gawd...", num_segment=-1)
        except AssertionError as error:
            assert str(error) == NON_POSITIVE_SEGMENT_MESSAGE
...
class TestCutByMileStone:
    def test_milestone_empty_text(self):
        assert _cut_by_milestone(text="", milestone=" ") == [""]
        assert _cut_by_milestone(text="", milestone="bobcat") == [""]

    def test_milestone_empty_milestone(self):
        try:
            _ = _cut_by_milestone(text="", milestone="")
            _ = _cut_by_milestone(text="The bobcat slept all day.",
                                  milestone="")
    ...
    def test_milestone_short_word(self):
        assert _cut_by_milestone(text="test\ntest", milestone="a") == [
            "test\ntest"]
        assert _cut_by_milestone(text="test\ntest", milestone="t") == [
            "", "es", "\n", "es", ""]
        assert _cut_by_milestone(text="ABAAB", milestone="A") \
               == ["", "B", "", "B"]

    def test_milestone_regular(self):
        text_content = "The bobcat slept all day.."
        milestone = "bobcat"
        assert _cut_by_milestone(text=text_content, milestone=milestone) == [
            "The ", " slept all day.."]
